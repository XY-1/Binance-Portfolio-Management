...
#'USDC', 'UNI', 'DOGE', 'XEM', 'AAVE', 'ATOM']
#'SOL','TRX', 'THETA', 'XTZ', 'VET', 'NEO', 'LUNA', 'BUSD', 'EGLD', 'ALGO',
#'SNX', 'DASH', 'FIL', 'SUSHI', 'MKR', 'AVAX', 'KSM', 'COMP', 'DCR',
#'ZEC', 'ETC', 'ZIL', 'BTT', 'CAKE', 'FTM', 'NEAR', 'RVN', 'YFI', 'UMA',
#'LSK', 'BAT', 'ENJ', 'IOST', 'QTUM', 'GRT']
...
def main():
	...
	manager = MAMRPortfolioManager(len(CURRENCIES) + 1, 2.105, 9.5, 1000, 0.000, 3)
	#manager2 = MAMRPortfolioManager(len(CURRENCIES) + 1, 2.105, 9.5, 1000, 0.000, 15)
	manager3 = MAMRPortfolioManager(len(CURRENCIES) + 1, 2.105, 9.5, 1000, 0.000, 26)
	manager4 = MAMRPortfolioManager(len(CURRENCIES) + 1, 2.105, 9.5, 1000, 0.000, 45)
	#manager = MAMRPortfolioManager(len(CURRENCIES) + 1, 50, 50, 60, 0.000, 26)

	for change, time in zip(price_changes, times):
		manager.value=(manager.value+manager3.value+manager4.value)/3
		#manager2.value=(manager.value+manager2.value+manager3.value+manager4.value)/3
		manager3.value=(manager.value+manager3.value+manager4.value)/3
		manager4.value=(manager.value+manager3.value+manager4.value)/3
		manager.update(time, change)
		#manager2.update(time, change)
		manager3.update(time, change)
		manager4.update(time, change)

	print((manager.value+manager3.value+manager4.value)/3)
